-- ============================================================
-- Forensic Medicine Department — 30 Analytical SQL Queries
...
FROM apps_postmortem_postmortem pm
JOIN apps_cases_forensiccase c ON pm.case_id = c.case_id
...
FROM apps_evidence_evidence ev
JOIN apps_cases_forensiccase c ON ev.case_id = c.case_id
...
FROM apps_cases_forensiccase c
JOIN apps_patients_patient p ON c.patient_id = p.patient_id
JOIN apps_staff_doctor d     ON c.doctor_id  = d.doctor_id
...
FROM apps_clinical_clinicalexamination ce
JOIN apps_cases_forensiccase c ON ce.case_id  = c.case_id
JOIN apps_patients_patient p   ON c.patient_id = p.patient_id
JOIN apps_staff_doctor d       ON ce.doctor_id = d.doctor_id
...
FROM apps_evidence_laboratorytest lt
JOIN apps_evidence_evidence ev    ON lt.evidence_id = ev.evidence_id
JOIN apps_cases_forensiccase c    ON ev.case_id      = c.case_id
...
FROM apps_reports_courtreport rpt
JOIN apps_cases_forensiccase c ON rpt.case_id  = c.case_id
JOIN apps_staff_doctor d       ON rpt.doctor_id = d.doctor_id
...
FROM apps_staff_staff s
LEFT JOIN apps_staff_doctor d ON d.staff_id = s.staff_id
...
FROM apps_staff_doctor d
LEFT JOIN apps_cases_forensiccase c ON c.doctor_id = d.doctor_id
...
-- Q13: Postmortem death type summary
SELECT death_type, COUNT(*) AS total_cases,
       ROUND(COUNT(*) * 100.0 / (SELECT COUNT(*) FROM apps_postmortem_postmortem), 1) AS percentage
FROM apps_postmortem_postmortem
GROUP BY death_type
ORDER BY total_cases DESC;
...
FROM apps_patients_patient p
JOIN apps_cases_forensiccase c ON c.patient_id = p.patient_id
GROUP BY p.district
ORDER BY total_cases DESC;
...
-- Q20: Court reports not yet submitted (Draft/Generated/Reviewed)
SELECT rpt.report_id, rpt.report_type, rpt.report_status,
...
-- Q22: Full case details from view
...
-- Q32: Gender breakdown of cases
SELECT p.gender, COUNT(c.case_id) AS total_cases,
       ROUND(COUNT(c.case_id) * 100.0 / (SELECT COUNT(*) FROM apps_cases_forensiccase), 1) AS percentage
FROM apps_cases_forensiccase c
JOIN apps_patients_patient p ON c.patient_id = p.patient_id
GROUP BY p.gender;

-- Q33: Cases overdue (> 30 days, not closed/archived)
SELECT c.case_number, c.case_type, c.case_status, c.priority,
       c.incident_date,
